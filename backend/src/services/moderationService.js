...
const AI_MODERATION_PROVIDERS = {
  ...
  google: {
    ...
    models: ['TOXICITY', 'SEVERE_TOXICITY', 'IDENTITY_ATTACK', 'INSULT', 'PROFANITY', 'THREAT'],
    categories: ['toxicity', 'severe_toxicity', 'identity_attack', 'insult', 'profanity', 'threat'],
    requiresApiKey: true
  },
  azure: {
    name: 'Azure Content Safety',
    endpoint: 'https://api.cognitive.microsoft.com/contentmoderator/moderate/v1.0/ProcessText/Screen',
    models: ['text'],
...
// AIモデレーション実行関数
async function performAIModeration(content, provider = 'openai', model = null, options = {}) {
  try {
    const providerConfig = AI_MODERATION_PROVIDERS[provider];
    if (!providerConfig) {
      throw new Error(`Unsupported AI moderation provider: ${provider}`);
    }
    ...
    switch (provider) {
      case 'openai':
        mockResult = {
          flagged: Math.random() > 0.7, // 30%の確率でフラグ
          categories: {
            'hate': { score: Math.random() * 0.3 },
            'harassment': { score: Math.random() * 0.4 },
            'sexual': { score: Math.random() * 0.2 },
            'violence': { score: Math.random() * 0.3 },
            'self-harm': { score: Math.random() * 0.1 }
          },
          category_scores: {
      ...
      case 'google':
        mockResult = {
          attributeScores: {
            'TOXICITY': { spanScores: [{ score: { value: Math.random() } }] },
            'SEVERE_TOXICITY': { spanScores: [{ score: { value: Math.random() * 0.5 } }] },
            'IDENTITY_ATTACK': { spanScores: [{ score: { value: Math.random() * 0.3 } }] },
            'INSULT': { spanScores: [{ score: { value: Math.random() * 0.4 } }] },
            'PROFANITY': { spanScores: [{ score: { value: Math.random() * 0.6 } }] }
          }
        };
        break;
      ...
      case 'azure':
        mockResult = {
          Classification: {
            Category1: { Score: Math.random() },
            Category2: { Score: Math.random() },
            Category3: { Score: Math.random() }
          },
          Terms: Math.random() > 0.8 ? [{ Term: 'inappropriate_word', Index: 10 }] : []
        };
        break;

      default:
        throw new Error(`No mock implementation for provider: ${provider}`);
    }

    // 結果を統一フォーマットに変換
    const normalizedResult = normalizeAIModerationResult(provider, mockResult);
    ...
    logger.warn('[AIModeration] Error performing AI moderation:', error);
    return {
      provider,
      available: false,
      error: error.message,
...
// AIモデレーション結果の正規化関数
function normalizeAIModerationResult(provider, rawResult) {
  const normalized = {
    score: 0,
    categories: {},
    flagged: false,
  ...
  try {
    switch (provider) {
      case 'openai':
        normalized.flagged = rawResult.flagged;
        normalized.categories = rawResult.category_scores;
        normalized.score = Math.max(...Object.values(rawResult.category_scores));
        normalized.details = rawResult;
        break;

      case 'google':
        normalized.categories = {};
        normalized.score = 0;
        Object.entries(rawResult.attributeScores).forEach(([category, data]) => {
          const score = data.spanScores[0]?.score?.value || 0;
          normalized.categories[category.toLowerCase()] = score;
          normalized.score = Math.max(normalized.score, score);
        });
        normalized.flagged = normalized.score > 0.7;
        normalized.details = rawResult;
        break;

      case 'azure':
        normalized.categories = {};
        if (rawResult.Classification) {
          Object.entries(rawResult.Classification).forEach(([category, data]) => {
            normalized.categories[category] = data.Score;
            normalized.score = Math.max(normalized.score, data.Score);
          });
        }
        normalized.flagged = rawResult.Terms && rawResult.Terms.length > 0;
        normalized.details = rawResult;
        break;
    ...
    logger.warn('[AIModeration] Error normalizing result for provider:', provider, error);
  }

  return normalized;
}

// 複数プロバイダーのAIモデレーションを実行
async function performMultiProviderAIModeration(content, providers = ['openai'], options = {}) {
  const results = [];
  const errors = [];

  for (const provider of providers) {
    try {
      const result = await performAIModeration(content, provider, null, options);
      results.push(result);
    } catch (error) {
      errors.push({ provider, error: error.message });
      logger.warn('[AIModeration] Provider failed:', provider, error);
    }
  }
  ...
  if (results.length > 0) {
    ...
    aggregated.aggregatedScore = scores.length > 0 ? scores.reduce((a, b) => a + b, 0) / scores.length : 0;
    ...
    allCategories.forEach(category => {
      ...
      aggregated.aggregatedCategories[category] = categoryScores.length > 0
        ? categoryScores.reduce((a, b) => a + b, 0) / categoryScores.length
        : 0;
    });

    // 全体的なフラグ判定（いずれかのプロバイダーがフラグした場合）
    aggregated.overallFlagged = results.some(r => r.flagged);

    // 合意レベルを判定
    const flaggedCount = results.filter(r => r.flagged).length;
    if (flaggedCount === results.length) aggregated.consensusLevel = 'high';
    else if (flaggedCount >= results.length / 2) aggregated.consensusLevel = 'medium';
    else aggregated.consensusLevel = 'low';
  }
