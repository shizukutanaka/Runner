 ...
 * Production-grade security implementation with rate limiting, input sanitization, and monitoring
 */
...
const registerCleanup = () => {
  ...
  const shutdown = async () => {
    try {
      ...
      logger.warn('[Security] Failed to disconnect Redis client', { error: error.message });
    }
  };
...
const getRedisClient = () => {
  ...
  try {
    ...
    client.on('error', (error) => {
      logger.error('[Security] Redis client error', { error: error.message });
    });
    client.connect().catch((error) => {
      logger.error('[Security] Failed to connect Redis client', { error: error.message });
    });
    sharedRedisClient = client;
    registerCleanup();
  } catch (error) {
    logger.error('[Security] Redis client initialization failed', { error: error.message });
...
const createRateLimiter = (options = {}) => {
  ...
  const limiterOptions = {
    ...
    handler: (req, res) => {
      ...
      const retryAfter = Math.min(windowMs / 1000, 3600);
