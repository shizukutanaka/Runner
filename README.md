...
### コミュニティインサイト（ソクラテス式問答による6つの新視点）
ソクラテス式問答法を用いて従来のモデレーション手法を根本から問い直し、6つの革新的な視点を実装：

1. **感情伝播検知（Emotional Contagion Detection）**
   - 「なぜ有害コメントは連鎖するのか？」→ 感情がウイルスのように伝播するため
   - 炎上を**後から削除**するのではなく、**起こる前に検知**
   - 直近20件のセンチメント勾配から炎上リスクを0-100%でスコアリング
   - 平均センチメント・感情速度・毒性スパイク・煽りユーザー検出を統合分析

2. **コミュニティ健全性スコア（Community Health Scoring）**
   - 「削除数が多いほど成功か？」→ 本当の成功は**介入が不要**な健全なコミュニティ
   - 単純な削除件数ではなく6つの多次元指標で健全性を測定
   - ポジティブ率・発言の深さ・参加多様性・モデレーション負荷・リピーター率・建設的会話率
   - S/A/B/C/Dグレード評価で一目で状態を把握

3. **クリエイター文化プロファイル（Creator Culture Profiles）**
   - 「誰がコミュニティ基準を決めるべきか？」→ クリエイター自身
   - ゲーム実況とファミリー向けでは許容度が異なる
   - 5つのプリセット：family（厳格）/educational/entertainment/gaming（寛容）/mature
   - チャンネルごとの文化に応じてモデレーションスコアを自動調整

4. **サイレント離脱検知（Silent Departure Detection）**
   - 「コミュニティ衰退の最初の警告は？」→ 常連が静かになること
   - 炭鉱のカナリア理論：鳴き止んだとき危険が来る
   - 常連ユーザーの沈黙期間を追跡し離脱リスクを早期警告
   - 新規減少よりも**常連の離脱**が深刻なシグナル

5. **モデレーター・トリアージ（Medical ER-style Triage）**
   - 「医師は患者を到着順に診るか？」→ 緊急度で分類する
   - コメントを到着順でなく**緊急度**で優先順位付け
   - EMERGENCY/URGENT/ROUTINE/CAN_WAITの4段階分類
   - チャンネルリスク状態も考慮した動的優先度算出

6. **コンテキスト認識分析（Context-Aware Analysis）**
   - 「同じ言葉でも文脈で意味が変わるのでは？」→ 前後の流れが判断を左右する
   - 単体評価と文脈考慮評価を比較
   - 炎上連鎖中は通常コメントも低評価、盛り上がり中はネガティブも緩和
   - コンテキスト依存度を可視化
...
- **セッション管理 / Session management**: セッション上限・有効期限管理、リフレッシュトークンローテーション（旧トークン即無効化）
- **異常検知 / Anomaly detection**: コメント流量の統計的異常検知（直近1時間 vs 24時間平均）と協調攻撃（ヘイトレイド）検知
...
- **多言語対応システム**: 日英2言語（i18n基盤は追加言語対応可能）、動的言語読み込み
...
- **セッション管理**: セッション上限・有効期限・リフレッシュトークンローテーション
