...
### E-46. ✅ 解決済み（2026-09-08） — 累犯カウントが時刻表現の不一致で違反を見逃す（ISOとSQLite時刻表現の不一致、再び）
...
- **教訓**: 「たまに失敗するテスト」を再実行して誤魔化さないこと。
  ...
  確認すること
- **次の問い**: 個々のバグを4件塞いだところで、視点を変えてみる。
  「それは誰に見えるのか」（E-40の問い）をもう一段深めると
  **「REST APIでは塞いだ認可の穴は、WebSocketにも同じように空いていないか？」**
  になる——実際に確認したところ、空いていた

---

### E-47. ✅ 解決済み（2026-09-08） — WebSocket層には身元確認が一切無く、他人のuserId宛の通知を横取り・注入できた

REST APIはE-40・E-41で認可の穴を塞いだ。次の問いは
**「同じ製品のWebSocket層にも、同じ種類の穴が空いていないか？」**である。
`ws.js`を読み直すと、**接続そのものを検証する仕組みが最初から存在しなかった**。

- **証拠1（room乗っ取り）**: `authenticate`イベントは、クライアントが
  自己申告する`data.userId`をそのまま使って
  `socket.join(\`user:${userId}\`)`していた。検証済みの身元が無いため、
  **他人のuserIdを名乗るだけでその人宛のroomに参加できる**。
  同一userIdを名乗った2つの接続は同じroomの2員になり、
  片方に送られた通知はもう片方にも届く
- **証拠2（未認証の注入）**: `sendNotification`イベントには
  **認証チェックも入力検証も一切無かった**。未認証の接続からでも、
  任意のuserId宛（`userId`省略時は接続中の**全員**へ）に
  任意の文言の「システム通知」を注入できた。フィッシングに悪用できる形である
- **証拠3（偽の操作の拡散）**: `moderationAction`イベントも同様に未認証で、
  実際のDB操作は伴わないものの「モデレーション操作が行われた」という
  偽の更新をダッシュボード全体へブロードキャストできた
- **実測**: 独立したNode.jsスクリプトで再現。攻撃者役の接続が
  `victim-user`を名乗るだけで、正規の`victim-user`宛の通知を
  **そのまま受信できた**ことを確認した
- **なぜREST側の対策だけでは防げなかったか**: E-40・E-41は
  `middleware/auth.js`のRESTミドルウェアの中身を直したが、
  `ws.js`は**別の入口**であり、そこには認可どころか認証の仕組み自体が
  最初から存在しなかった。一つの入口を固めても、別の入口が
  素通しならば意味が無い
- **実施した対応**:
  - `io.use()`接続ミドルウェアを追加。REST APIと同じhttpOnly Cookie
    （`access_token`）をWebSocketハンドシェイクのHTTPリクエストヘッダーから読み、
    REST側と同じ`verifyToken`で検証して`socket.user`に載せる。
    フロントの`socket.io-client`は元々`withCredentials: true`で接続しており、
    **Cookieは既に自動送信されていたため、フロント側の変更は不要**だった。
    非ブラウザクライアント（テスト等）向けに`socket.handshake.auth.token`も
    受け付ける
  - `authenticate`は`socket.user.id`（検証済み）を使い、
    クライアントが自己申告する`userId`は無視するよう変更。
    `socket.user`が無ければroomには参加させず`error`を返す
  - `sendNotification`・`moderationAction`は`socket.user.role`が
    moderator以上でなければ拒否するようにした（`sendNotification`には
    入力検証とレート制限も無かったため、あわせて追加した）
  - `verifyToken`と`ROLE_HIERARCHY`を`middleware/auth.js`からexportし、
    REST側と全く同じ検証ロジック・同じ役割序列をWebSocket層でも使う
    （検証方法が2箇所に分かれて食い違う将来を防ぐ）
- **ガード**: `backend/tests/integration/websocketAuth.test.js`（5件）—
  実サーバーを起動し`socket.io-client`で接続する統合テスト。
  自己申告のuserIdでは他人のroomに参加できないこと（正規のvictimは
  通知を受け取るが、victimを名乗っただけの攻撃者は受け取らないこと）、
  未認証接続の`authenticate`が拒否されること、未認証接続からの
  `sendNotification`が他人に届かないこと、moderator未満の役割からは
  `sendNotification`/`moderationAction`が拒否されること、moderator以上なら
  通ること（締めすぎ防止）。旧実装に戻すと5件中4件が落ちることを確認済み
- **テスト自体の欠陥に気づいた経緯**: 最初に書いたテストは、
  victim側の受信を`await`で待ってから攻撃者側を確認する形にしており、
  **ほぼ同時に2つの接続へ届くイベントの一方を待っている間に、
  もう一方がリスナー登録前に届いて消費されずに落ちる**という競合により、
  旧実装（脆弱な状態）でも誤ってpassしていた。独立したデバッグスクリプトで
  「攻撃者も実際には通知を受信している」ことを直接確認して発覚し、
  アクションを起こす前に両方へリスナーを仕込む形に直した。
  **「テストが通ったから直っている」を鵜呑みにせず、
...
### E-51. ✅ 解決済み（2026-09-08） — 死蔵コード検査自体に「同名の別の死蔵コードを参照とみなす」穴があり、丸ごと架空のAIチャットボット設定タブと、ルート登録漏れの設定APIを1つずつ見逃していた

E-50で「テストが無い画面」を1つずつ調べ始めた流れで、SettingsPanelを読み直した。
`settings_tab_ai`（既存のAI設定タブ）とは別に「AIチャットボット設定」という
7項目以上のタブが丸ごとあり、有効化スイッチ・自動応答・信頼度閾値・
レート制限・対応言語・パーソナリティ（トーン/敬語レベル/ユーモア度）まで
作り込まれていた。**呼ばれるか？**を当ててみた。

- **証拠**: `settings.chatbot.*`を書き込む経路（`SettingsPanel.js`の7項目）は
  存在するが、**それを読むコードがバックエンド・フロントエンドのどこにも無かった**。
  バックエンドには`generateChatbotResponse`の実装が**2つ**あった
  （`moderationService.js`のFAQ/製品推薦/定型応答ツリーと、
  `openaiService.js`のGPT応答生成）が、**両方とも呼び出し元がゼロ**。
  さらに、たとえ呼ばれても**送信先が無い**——YouTube/Twitchへメッセージを
  投稿する経路（`liveChatMessages.insert`相当）はこのプロダクトに
  一つも実装されていない（D-2はコメントの**取り込み**のみ実装済みで、
  書き戻し全般は明文化された既知の未着手）。つまりこの設定タブは、
  仮に完全に配線しても**技術的に何も配信されようがない**状態だった
- **なぜ死蔵エクスポート検査（`deadExports.test.js`）が見逃していたか**:
  同検査は「エクスポートされた名前が他のどのファイルにも一切現れなければ
  死蔵」と判定する素朴な文字列一致だった。ところが`generateChatbotResponse`
  という同じ名前が、`moderationService.js`側にも**別の、これまた誰にも
  呼ばれない内部関数**として存在していたため、互いの存在そのものが
  「参照あり」の証拠として誤認され、どちらも検出対象からすり抜けていた。
  E-33でフロントの死蔵コード検査に見つかった「名前のgrepでは同名の別物に
  一致して未使用を見逃す」という穴と、**まったく同じ形**が今度は
  バックエンドの検査自体に潜んでいた
- **実施した対応**:
  1. `deadExports.test.js`の`referencedElsewhere`を修正。他ファイルの
     テキストから「その名前自身の宣言行」（`function NAME`・`const/let/var NAME =`・
     `exports.NAME =`）を取り除いてから参照の有無を判定するようにし、
     宣言同士が互いを参照し合ったことにしてしまう誤判定を防いだ
  2. 修正後の検査を実行し直したところ、チャットボット関連に加えて
     **もう1件、別の欠陥（後述）も同時に見つかった**——検査の穴を塞いだ
     瞬間、それまで隠れていた実害が2件とも表に出てきた
  3. 送信先の無い機能を「実装済みに見えるが実は何もしない」状態のまま
     残すのは、この検査自身が冒頭コメントで警告している通り
     「読んだ人が『この機能はある（あとは繋ぐだけ）』と信じる」害そのものであり、
     かつ「未実装の機能を動作しているように見せない」という本セッションの
     不変条件に反する。オーナー判断が要る新機能（実際のチャット送信）を
     勝手に作り込む代わりに、`SettingsPanel.js`から「AIチャットボット設定」
     タブを丸ごと削除し、`moderationService.js`のFAQ/製品推薦ツリーと
     `openaiService.js`のGPT応答生成、および誰も読んでいなかった
     `CACHE_TTL_MS.chatbot`設定を削除した（E-33・E-37・E-48と同じ判断:
     死蔵コードは「将来のために残す」のではなく削除する）
  4. 修正後の検査が同時に見つけたもう1件: `settingsController.setDisplay`
     （フォントサイズ・表示密度・メディア表示等を保存する、完成した
     ハンドラ）と`validation/settings.setDisplay`（対応するJoiスキーマ）は
     どちらも実装済みで、既存の約30個の兄弟エンドポイント（`theme`・
     `layout`・`notifications`等）と全く同じ形をしていたが、
     **`routes/settings.js`に一度も登録されておらず、PUTすると404だった**。
     これは同じ「同名の別死蔵コードが互いを隠す」構造で見えなくなっていた
     （コントローラー側とバリデーション側、どちらも「相手に参照されている」
     と誤認されていた）。こちらはチャットボットと違って**構造的に完成しており、
     兄弟エンドポイントと同じ土台（`updateNestedSetting`ヘルパー・
     `user_settings`テーブル）で動くことが明らかだったため、削除ではなく
     兄弟と同じ形でルートを1行追加して配線した**
- **ガード**:
  - `backend/tests/architecture/deadExports.test.js`の修正自体が今後の
    同型再発を防ぐ恒久的なガードである。旧ロジックに戻すと、チャットボットの
    死蔵コードを復元した状態でもこの検査が誤って合格することを確認済み
    （`git stash`相当の復元＋旧ロジックで検査 → 緑、新ロジックで検査 → 赤、
    の両方を実測した）
  - `backend/tests/api/settings.test.js`に`PUT /api/settings/user/:userId/display`
    の3件（正常系・不正なフォントサイズ・不正な表示密度）を追加。
    ルート登録を外すと3件とも404で失敗することを確認済み
- **実測**: backend 787件（784+3）/ frontend 128件、失敗0・skip 0
- **再検証**: `cd backend && npx jest tests/architecture/deadExports.test.js tests/api/settings.test.js`
- **次の問い**: 「死蔵コード検査」自身に穴があったという事実は、
