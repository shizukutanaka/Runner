...
難易度別:
  ...
  indirect        4件  検知率  25.0%
  hard-negative   7件  誤検知 0/7
```

**残る見逃し3件はいずれも決定論的ルールで扱うべきではないもの**である。

- i1「その顔で配信とか、よく人前に出られるね」— 丁寧語で包んだ容姿侮辱
- i2「誰も見てないのに続ける意味あるの？やめたら？」— 活動否定
- i4「この配信者、前に問題起こしてたって本当ですか？みんな知ってる話で」— 噂の流布

これらを語彙や正規表現で捕まえようとすると、
「その顔文字かわいい」「意味あるの？」「本当ですか？」といった
無害な発話を大量に巻き込む。Precision 100% を捨てることになる。
**ここから先はAI層（Policy-as-Prompt, W-3）の担当であり、
OPENAI_API_KEY を設定した状態で `node src/scripts/evaluateModeration.js` を
