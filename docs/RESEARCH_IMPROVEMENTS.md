...
3. **in-memory状態の揮発**: ~~文化プロファイル~~ → **R-18で文化プロファイルはDB永続化済み**（2026-07-18）。離脱検知の追跡データは依然in-memory（次段）
...
- **残課題**: 離脱検知（`silentDepartureDetector.js`）の追跡データも同様にin-memory。同じパターンで永続化可能だが今回は未着手

---

## 検証（本ブランチで実施済みの R-1/R-3/R-5a/R-5補足/R-10/R-11/R-12/R-14/R-18）
