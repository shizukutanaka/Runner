...
## R-35. 属性差別の検知と、決定論的パターンの天井（ホールドアウトで効果ゼロ）

**やったこと**: R-34 で見えた「本物の弱点18件」の大半が
**属性への言及 + 一般化された貶め**という共通構造を持っていたので、
R-31/R-33 と同じ2成分共起（＋20文字以内の近接）で拾いにいった。

外部セットを見ながら調整すると訓練データになってしまうため、
**セットを機械的に二分（idの偶奇）し、dev側だけを見て設計し、
test側は最後の報告まで一度も見なかった**。
（第2の外部セットを探したが、公開されているものを見つけられなかったための次善策）

**結果**:

| | 調整前 | 調整後 | 差 |
|---|---|---|---|
| dev（見ながら調整した側） | Recall 7.1% | **35.7%** | +28.6pt |
| **test（一度も見ていない側）** | Recall 13.3% | **13.3%** | **±0** |

**ホールドアウトでの改善はゼロだった。**

dev で +28.6ポイント、test で ±0。これは典型的な過学習の形であり、
書いたパターンが dev の個々の文には一致しても、**現象そのものを捉えていない**
ことを意味する。もし dev だけを見て「Recall が5倍になった」と報告していたら、
それは完全な誤りだった。

**結論（この製品の方針として）**:
**決定論的なパターンで一般的な有害表現のカバレッジを上げる方針は天井に当たっている。**
R-30/R-31/R-33 が自作セットで効いたのは、
「配信チャットに特有の、構造がはっきりした攻撃」に的を絞っていたからであって、
一般的な有害表現の長い裾野は語彙と正規表現では覆えない。
ここから先の広いカバレッジは**学習済みモデル（AI層）の担当**である。
——これは R-30 の時点でも書いていたが、当時は根拠のない見込みだった。
今回は**測って確かめた**。

**それでも R-35 を削除していない理由**:
- 拾えている発話（「働けよ無職」「中国人って素行が悪いからなぁ」「士業は路頭に迷え」）は
  配信のモデレーターが確実に見たいものであること
- 誤検知が **dev 141件 / test 139件 / 自作 29件 / 配信チャットのプローブ 9件のすべてで 0件**、
  つまりコストがゼロであること

モデレーションの語彙は本質的に長い裾野を持つので、
「狭いが正確」なルールを積むこと自体は正当である。
ただし**これで広くカバーできるとは考えないこと**。
...
node src/scripts/evaluateModeration.js --file=dev.json    # 調整に使ってよい側
node src/scripts/evaluateModeration.js --file=test.json   # 報告用。調整に使わないこと
```

**今後のセッションへ**: test側を見ながらパターンを調整した瞬間、
このプロジェクトは**独立した検証の場を完全に失う**。
分割は id の偶奇という決定的規則なので、都合よく引き直すこともできない。

## R-36. 性的ハラスメントの検知（存在しなかったカテゴリ）と、R-35との対照

**動機**: 配信者への性的ハラスメントは、ライブ配信のモデレーションで最も報告の多い
加害のひとつ（特に女性配信者）であるにもかかわらず、**本製品にはこの区分が
そもそも存在しなかった**。NGワードのカテゴリは abuse / threat / spam / discrimination の
4つだけで、性的な加害はどこにも分類されない。

しかも文化プロファイル側には `no_adult`（family）/ `adult_verified`（mature）という
**フラグが既にあった**。つまり「成人向け表現を制御する」足場は用意されていたのに、
**それが働きかける対象が無い**状態だった。

**実装**:

- `ng-words.json` に `sexual` カテゴリを新設（ja 14語 / en 4語）。
  明示的な加害語のみを入れ、**文脈次第で無害になりうる語は入れていない**
- 語彙で扱えない部分は2成分の共起で拾う（`harassment:sexual`）:
  身体への言及（胸・下着・裸 等）× 性的な要求・評価（見せて・触りたい・舐め 等）、
  かつ**20文字以内の近接**。ゲーム配信で普通に出る
  「このキャラの胸当て強い」「脚が速いキャラ」に当たらないようにするため
- 検知結果は他と同じく**保留（人間のレビュー）**に回すのみ。自動処罰はしない。
  成人向けチャンネルでの扱いは文化プロファイルの `bannedPatternBoost`（mature=0.7）が
  スコア側で調整する

---

### R-35 との対照 —— これがこのプロジェクトで一番重要な比較

同じ「2成分共起」という手法を使いながら、**結果が正反対**になった。

| | 設計の出発点 | dev（調整に使った側） | **test（一度も見ていない側）** |
|---|---|---|---|
| **R-35** 属性差別 | **外部セットの見逃しを見て**設計 | 7.1% → 35.7%（**+28.6pt**） | 13.3% → 13.3%（**±0**） |
| **R-36** 性的ハラスメント | **製品ドメインの知識から**設計 | 35.7%（変化なし） | 13.3% → **20.0%（+6.7pt）** |

R-35 は dev で5倍になりながら**ホールドアウトでは1件も増えなかった**。
R-36 は dev では動かず、**ホールドアウトで実際に増えた**。

手法は同じである。違ったのは**何を見て設計したか**だけ:

- **データを見て設計すると、そのデータにしか効かないものができる**（R-35）
- **問題領域を理解して設計すると、見ていないデータでも効く**（R-36）

評価セットを見ながらパターンを書き足す作業は、進捗しているように見えて
実際には何も一般化していない可能性がある。**dev で上がった数字を成果として
報告してはならない。報告してよいのは test 側の数字だけである。**

**現時点の実測**:

```
自作セット(45件)         Precision 100.0% / Recall 100.0% / F1 100.0%
外部 dev (155件)         Precision 100.0% / Recall  35.7% / F1  52.6%
外部 test(154件・未使用) Precision 100.0% / Recall  20.0% / F1  33.3%
                         無害139件に対する誤検知 0件
```

Precision は3セットすべてで 100%、誤検知は一貫して0件を維持している。
「無害な発話を落とさない」という設計目標は、他人が書いたデータでも守られている。

**テスト**: `tests/services/sexualHarassment.test.js`（21件）。
検知9件に対し**過剰検知ガード10件**＋保留キューへの到達2件。
ガード側には「胸当て」「エロゲー実況」「下着メーカーの案件」など、
配信チャットで実際に出る言い回しを入れてある。
