...
難易度別:
  direct          5件  検知率 100.0%
  evasion         4件  検知率 100.0%
  indirect        7件  検知率 100.0%
  hard-negative  29件  誤検知 0/29
```

**この数値の読み方（重要）**: パターンを書いた人間が評価ケースも書いている以上、
100%は**一般化の証拠ではなく整合性の証拠**である。実際、R-33の初版は自作の
評価セットで F1 100% を出しながら、設計時に想定していなかった実在しそうな
チャット8件のうち **5件を誤検知**した。そこで hard-negative を 29件まで厚くし、
うち14件は**パターンを見ずに**「実際のチャットならこう書く」という観点で
書き出したものを入れてある（初版が誤検知した5件を含む）。
それでも、これは45件の内輪の物差しにすぎない。

**外部ベンチマークでの検証が次の一手**である。WildGuardTestJP / AnswerCarefully 等は
ライセンス・配布条件のため同梱していないが、`src/data/moderation-eval-set.json` と
同じ形式に変換すれば `--file=` でそのまま測れる。
