...
| # | 内容 | 参照 |
|---|---|---|
| ~~0~~ | ~~モデレーション判断がプラットフォームへ書き戻されない~~ → **R-28でコード実装完了**（2026-08-15）。削除の書き戻し・OAuth2認証・クォータ会計・透明な結果報告まで実装済みで、**残るは資格情報の設定のみ**（`YOUTUBE_CLIENT_ID`/`SECRET`/`REFRESH_TOKEN`）。**BAN書き戻しも配線済み**（R-28b）。残るは保留却下経路のみ | R-20 / R-28 |
