...
4. **テスト運用規律** — 530テスト・**失敗0件**（2026-08-15にsettings4件が解消）。「ベースライン悪化ゼロ」を毎変更で確認する文化が確立
...
- **テストベースライン**: `cd backend && rm -f data/test.db && NODE_ENV=test npx jest --runInBand` → **0 failed / 520 passed / 530 total（10 skipped）** が基準（2026-08-15時点。この数値は増分ごとに更新すること）。**長らく据え置かれていた `tests/api/settings.test.js` の4件はリポジトリオーナーの `5209f8e fix(settings)` で解消され、全件グリーンになった。以後は「失敗0」が基準であり、1件でも失敗させる変更は禁止**。並列実行は避け、**`--runInBand` + 事前の `rm -f data/test.db`** を使うこと（オーナーの `04f8914` で実行ごと一意なテストDB名が導入されたが、規約は維持する）
