...
4. **テスト運用規律** — 539テスト・**失敗0件**（2026-08-15にsettings4件が解消）。「ベースライン悪化ゼロ」を毎変更で確認する文化が確立
...
- **不変条件**: 全工程でテスト失敗0を維持（現在 529 passed）
...
- `config.js` の `rateLimit` に `enabled`（既定=本番のみ有効・`RATE_LIMIT_ENABLED`で上書き可）/`store`/`general`/`api`/`strict` を追加。テスト・dev環境では既定で無効のためベースライン不変。実サーバーで429を確認。`tests/middleware/rateLimit.test.js` に回帰テスト2件。詳細は FEATURE_AUDIT E-14
