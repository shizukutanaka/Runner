...
4. **テスト運用規律** — **697テスト・失敗0件・skip 0件**（2026-09-01時点）。「ベースライン悪化ゼロ」を毎変更で確認する文化が確立
...
8. **業務ワークフロー** — 保留キュー（理由バッジ付き）・ソフトデリート+削除履歴の監査証跡・モデレータートリアージ

## 1.5 完成状態（2026-08-15・イーロン・マスク式アルゴリズム適用後）

**製品の因果連鎖は全段階が実装完了**した:

| 段階 | 状態 |
|---|---|
| ①取込 | ✅ YouTube（ポーリング/クォータ管理）＋ **Twitch（EventSub WebSocket・Shared Chat対策込み・R-29）** |
| ②判定 | ✅ NGワード＋回避対策＋omni-moderation＋Policy-as-Prompt（助言層）＋レイド検知＋累犯エスカレーション |
| ③**実行** | ✅ **削除・BAN・保留却下の3経路すべてでプラットフォームへ書き戻し**（R-28/b/c）。透明な結果報告＋フェイルセーフ付き |
| ④人間の介在 | ✅ 保留キュー（構造化理由バッジ）・トリアージ・手動オーバーライド |
| ⑤記録 | ✅ 監査証跡・削除履歴 |

**適用したステップと成果**:
- **ステップ1（要求を疑う）**: 「OAuth資格情報が無いから書き戻しは無理」を分解し、**資格情報とコードは別問題**と判明→コードを完成させた（R-28）。「Twitch対応」は製品名が約束する要求と確認→実装した（R-29）
- **ステップ2（削除）**: 未参照モジュール13個（5,419行）、偽機能2つ（papers/related-videos）、**捏造データを返す34ハンドラ**を削除。backend 37,538→31,641行（**約6,000行削減**）
- **不変条件**: 全工程でテスト失敗0を維持（現在 697 passed / skip 0）

**フロントエンドの監査で見つかった最大の欺瞞（2026-08-18・修正済み）**:
`ModeratorDashboard` の **BAN/ミュート/削除/非表示ボタンは一切APIを呼んでいなかった**。
ローカル配列に「実行済み」の見た目だけを追加しており、モデレーターは操作が成功したと
信じるが、DBにもプラットフォームにも何も起きない。**製品の主要モデレーション画面が
モデレーションしていなかった**。実APIへ配線し、失敗はSnackbarで明示、
プラットフォーム反映の可否も併記するようにした（実ブラウザでDB反映まで確認済み）。
同じ検査を全コンポーネントに適用し、他に該当なしを確認。

**残るのは「コードで解決できない」ものだけ**:
1. **資格情報**: `YOUTUBE_CLIENT_ID/SECRET/REFRESH_TOKEN`（書き戻し有効化）、`TWITCH_USER_ACCESS_TOKEN/USER_ID`（Twitch取込有効化）、`OPENAI_API_KEY`（LLM判定）— いずれも設定すればコード変更なしに有効化される
2. **製品/法務judgment**: マルチテナント（D-6/E-3）・httpOnly移行（D-7）・規制対応（R-13）・Stripe課金UI（E-5）— **ユーザーの明示指示があった場合のみ着手**

## 2. 短所（残存課題のみ・2026-07-18時点）

| # | 内容 | 参照 |
|---|---|---|
| ~~0~~ | ~~モデレーション判断がプラットフォームへ書き戻されない~~ → **R-28でコード実装完了**（2026-08-15）。削除の書き戻し・OAuth2認証・クォータ会計・透明な結果報告まで実装済みで、**残るは資格情報の設定のみ**（`YOUTUBE_CLIENT_ID`/`SECRET`/`REFRESH_TOKEN`）。**削除・BAN・保留却下の3経路すべて配線完了**（R-28/b/c）。**残るは資格情報の設定のみ** | R-20 / R-28 |
| ~~1~~ | ~~Twitch未実装~~ → **R-29で実装完了**（2026-08-15）。EventSub WebSocket + `channel.chat.message`、Shared Chatの二重処理対策込み。**残るは実クレデンシャルでの疎通確認のみ** | R-7 / R-29 |
| ~~2~~ | ~~レート制限が全体で無効~~ → **W-1で解決済み**（2026-07-18・本番のみ既定有効） | E-14 / W-1 |
| ~~3~~ | ~~スタブAPI約48関数がハードコード値を返す~~ → **解消済み**。analyticsController は実クエリ化/501（W-4）、moderationController は**捏造データを返す32ハンドラを削除**（2,262→934行）。`totalLinksDetected:1247` 等の偽統計は全て除去 | E-1/E-2 / W-4 |
...
- **テストベースライン**: `cd backend && npx jest --silent` → **0 failed / 697 passed / 697 total（skip 0）** が基準（2026-09-01時点。増分ごとに更新すること）。テストDBは実行ごとに一意なファイル名が自動生成されるため事前削除は不要。**失敗0が基準であり、1件でも失敗させる変更は禁止**
...
- **並列実行について**: オーナーの `04f8914` で実行ごと一意なテストDB名が導入され、**並列実行でも全件通過することを確認済み**（CIは並列で実行される）。`--runInBand` は再現性重視の推奨に留まり、必須ではなくなった
...
### W-1. レート制限の有効化（E-14）✅ 完了（2026-07-18）
- `config.js` の `rateLimit` に `enabled`（既定=本番のみ有効・`RATE_LIMIT_ENABLED`で上書き可）/`store`/`general`/`api`/`strict` を追加。テスト・dev環境では既定で無効のためベースライン不変。実サーバーで429を確認。回帰テストは `tests/api/rateLimit.test.js`（E-19で移設・実リミッター起動型に強化）。詳細は FEATURE_AUDIT E-14
...
### W-5. 日本語評価ハーネス（R-5b）完了（2026-08-08）
- `node src/scripts/evaluateModeration.js` で Precision/Recall/F1 を difficulty 別に測定。**実測: F1 72.7%（direct 100% / evasion 75% / indirect 0% / hard-negative 誤検知1）**
- **この数値はW-3（R-4 Policy-as-Prompt）の優先度の根拠**: 語彙に頼らない攻撃が0%であり、語を足しても解決しない（誤検知が増えるだけ）ことが実証された。W-3実装後は同ハーネスで効果を測ること
- 回帰テスト `tests/services/moderationEval.test.js` が現状値を下限として固定済み
...
### W-13. `papers` の偽の約束を解消（R-21残課題）✅ 完了 → その後 削除（2026-08-25）
- 経緯: まず4エンドポイントを **501 + `implemented:false`** にした（W-4と同じ規約）。
  案内していた `SEMANTIC_SCHOLAR_API_KEY` は**読むコードが存在しなかった**ため、
  「キーを設定すれば有効」という旧メッセージ自体が誤りだった
- **現在は関連ファイルごと削除済み**。`routes/papers.js` も `RelatedPapersDialog.js` も
  リポジトリに存在しない。501を返し続けるより、無い機能はUIごと消す方が正直である
  （経緯は `docs/FEATURE_AUDIT.md`）

## 7. 定型コマンド集

```bash
# テスト（ベースライン確認）。テストDBは実行ごとに一意名で作られるので事前削除は不要
cd backend && npx jest --silent 2>&1 | grep -E "Test Suites:|Tests:"

# devサーバー起動（`npm run dev` でよい。predev の checkEnv は警告のみで続行する）
