...
4. **テスト運用規律** — **658テスト・失敗0件・skip 0件**（2026-09-01時点）。「ベースライン悪化ゼロ」を毎変更で確認する文化が確立
...
- **不変条件**: 全工程でテスト失敗0を維持（現在 658 passed / skip 0）
...
- **テストベースライン**: `cd backend && npx jest --silent` → **0 failed / 658 passed / 658 total（skip 0）** が基準（2026-09-01時点。増分ごとに更新すること）。テストDBは実行ごとに一意なファイル名が自動生成されるため事前削除は不要。**失敗0が基準であり、1件でも失敗させる変更は禁止**
...
### W-13. `papers` の偽の約束を解消（R-21残課題）✅ 完了（2026-08-15）
- 4エンドポイントすべてを **501 + `implemented:false`** に変更（W-4と同じ規約）。案内していた `SEMANTIC_SCHOLAR_API_KEY` は**読むコードが存在しない**ことが判明したため、「キーを設定すれば有効」という旧メッセージ自体が誤りだった
- フロントは既存のエラー経路がメッセージを表示するため改修不要。実ブラウザで「未実装」表示を確認済み
- 以下は旧記載（対応済み）:
