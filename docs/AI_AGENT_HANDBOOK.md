...
4. **テスト運用規律** — **756テスト・失敗0件・skip 0件**（2026-09-02時点）。「ベースライン悪化ゼロ」を毎変更で確認する文化が確立
...
- **不変条件**: 全工程でテスト失敗0を維持（現在 756 passed / skip 0）
...
- **テストベースライン**: `cd backend && npx jest --silent` → **0 failed / 756 passed / 756 total（skip 0）** が基準（2026-09-02時点。増分ごとに更新すること）。テストDBは実行ごとに一意なファイル名が自動生成されるため事前削除は不要。**失敗0が基準であり、1件でも失敗させる変更は禁止**
